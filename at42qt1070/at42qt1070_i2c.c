...
bool at42qt1070_touched() {
  uint8_t buf[1];
  i2c_writing_blocking(i2c_default, ADDR, AT42QT107_DETECT_STATUS, 1, true);
...
uint8_t at42qt1070_key_touched() {
  uint8_t buf[1];
  i2c_write_blocking(i2c_default, ADDR, AT42QT107_KEY_STATUS, 1, true);
  i2c_read_blocking(i2c_default, ADDR, buf, 1, false);
  return buf[0];
}
